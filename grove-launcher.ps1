...
if (-not (Test-Path $GroveRoot)) {
    ...
    Write-Host "  Make sure Z: drive is mapped to \\THE-BATMAN\mnt`n" -ForegroundColor Yellow
...
switch ($Selection.ToLower()) {
    ...
    '1' {
        ...
        Write-Host "`n  Launching Claude Code...`n" -ForegroundColor Green
        claude --dangerously-skip-permissions
    }
    '2' {
        Set-Location $GroveRoot
        Write-Host "`n  Launching Claude Code (autonomous)...`n" -ForegroundColor Green
        claude --dangerously-skip-permissions -p "/gsd:autonomous"
    }
    '3' {
        Set-Location $GroveRoot
        Write-Host "`n  Starting Tauri dev server...`n" -ForegroundColor Green
        cargo tauri dev
    }
    '4' {
        Set-Location $GroveRoot
        Write-Host "`n  Building release...`n" -ForegroundColor Green
        cargo tauri build
    }
    default {
