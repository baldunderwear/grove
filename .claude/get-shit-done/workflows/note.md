...
- **Global scope**: `Z:/data/development/forge/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `Z:/data/development/forge/.claude/notes/`)
...
2. Glob `Z:/data/development/forge/.claude/notes/*.md` (if directory exists) — global notes
...
Global (Z:/data/development/forge/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `Z:/data/development/forge/.claude/notes/` — works in any directory
